    ...
    try {
        ...
        fs.access(targetPath, constants.F_OK, async function (error) {
            if (error) { //directory does not exist
                ...
                next(new CustomError("FILE", 404, "해당 경로에 이미 같은 이름의 디렉토리가 존재합니다"))
            }
        });
    } catch (err) {
        next(new CustomError("FILE", 404, "디렉토리 생성에 실패하였습니다."))
    }
})
...
router.patch('/', async (req, res, next) => {
    try {
        ...
        fs.access(newPath, constants.F_OK, function (error) {
            if (error) { //directory does not exist
                fs.rename(targetPath, newPath, (err) => {
                    if (err) {
                        return  next(new CustomError("FILE", 404, "파일/폴더명 변경에 실패하였습니다. (No such file or directory)"))
                    }
                    console.log("hello")
                    res.status(200).json(
                        {
        ...
        next(new CustomError("FILE", 404, "파일/폴더명 변경에 실패하였습니다."))
    }
